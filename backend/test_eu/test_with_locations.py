"""
Test epistemic unit with LOCATION entities included as anchors.

Hypothesis: For events like fires, the location IS the anchor that
connects claims across different aspects (death toll, response, investigation).
"""
import asyncio
import os
...
# Hub locations to exclude (too generic)
...
async def test():
    ...
    anchor_counts = {}
    for c in all_claims:
        for a in c.anchor_entities:
            anchor_counts[a] = anchor_counts.get(a, 0) + 1
    for a, cnt in sorted(anchor_counts.items(), key=lambda x: -x[1])[:10]:
        print(f'  {a}: {cnt} claims')
    ...
    for s in surfaces:
        ...
        print(f'  {s.id}: {len(s.claim_ids)} claims, anchors={list(s.anchor_entities)[:3]}, event={ev_short} [{purity_tag}]')
    ...
    for s1, s2, score, ev in aboutness:
        ...
        anchors = ev.get("shared_anchors", [])[:2]
        print(f'  {s1}<->{s2}: {score:.2f} sig={ev.get("signals_met",0)} anchors={anchors} [{ev1}<->{ev2}] {tag}')
