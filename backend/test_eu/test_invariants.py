...
from core.epistemic_unit import (
    Claim, Surface, Event, AboutnessLink,
    Relation, Association,
    Parameters, ParameterChange, MetaClaim,
    EmergenceEngine,
    cosine_similarity, jaccard, temporal_overlap
)
...
@pytest.fixture
def engine():
    """Fresh EmergenceEngine for testing."""
    return EmergenceEngine(llm=None)
...
class TestInvariant1_L0Immutability:
    ...
    async def test_claim_text_immutable(self, sample_claims):
        ...
        assert engine.claims[claim.id].text == original_text
    ...
    async def test_claim_provenance_preserved(self, sample_claims):
        ...
        assert engine.claims[claim.id].source == original_source
...
class TestInvariant2_ParameterVersioning:
    ...
    def test_engine_uses_parameters(self):
        ...
        assert engine.params.hub_max_df == 10
        assert engine.params.version == 2
...
class TestInvariant3_IdentityAboutnessSeparation:
    ...
    async def test_surfaces_use_identity_edges_only(self, sample_claims):
        ...
        engine.claim_edges.append(("c1", "c2", Relation.CONFIRMS, 0.9))
    ...
    async def test_aboutness_does_not_merge_surfaces(self, sample_claims):
        ...
        engine.claim_edges.append(("c1", "c2", Relation.CONFIRMS, 0.9))
        engine.claim_edges.append(("c4", "c5", Relation.CONFIRMS, 0.9))
...
class TestInvariant4_DerivedStatePurity:
    ...
    async def test_surfaces_derived_from_claims(self, sample_claims):
        ...
        surfaces1 = engine.compute_surfaces()

        # Recompute should give same result
        engine.surfaces = {}  # Clear
        engine._surface_counter = 0
        surfaces2 = engine.compute_surfaces()
    ...
    async def test_events_derived_from_surfaces(self, sample_claims):
        ...
        engine.compute_surfaces()
        engine.compute_surface_aboutness()
        events1 = engine.compute_events()

        # Recompute should give consistent result
        engine.events = {}
        engine._event_counter = 0
        events2 = engine.compute_events()
    ...
    async def test_parameter_change_affects_derived_state(self, sample_claims):
        ...
        engine.compute_surfaces()
        engine.compute_surface_aboutness()
        aboutness_count_1 = len(engine.surface_aboutness)

        # Change parameter
        params.update("aboutness_min_signals", 3, actor="test")

        # Recompute with stricter threshold
        engine.surface_aboutness = []
        for s in engine.surfaces.values():
            s.about_links = []
        engine.compute_surface_aboutness()
        aboutness_count_2 = len(engine.surface_aboutness)
...
class TestInvariant5_StableCoreRelations:
    ...
    async def test_only_core_relations_in_edges(self, sample_claims):
        ...
        engine.claim_edges.append(("c1", "c2", Relation.CONFIRMS, 0.9))
        engine.claim_edges.append(("c2", "c3", Relation.REFINES, 0.8))
        engine.claim_edges.append(("c4", "c5", Relation.CONFLICTS, 0.7))
...
class TestInvariant6_MetaClaimsObservations:
    ...
    async def test_meta_claims_can_be_resolved(self, sample_claims):
        ...
        if meta_claims:
            ...
            engine.resolve_meta_claim(mc.id, "new_claim_added", actor="test")
    ...
    async def test_unresolved_meta_claims_filter(self, sample_claims):
        ...
        unresolved = engine.get_unresolved_meta_claims()
        assert len(unresolved) == len(meta_claims)

        # Resolve one
        if meta_claims:
            engine.resolve_meta_claim(meta_claims[0].id, "dismissed", actor="test")
            unresolved = engine.get_unresolved_meta_claims()
...
class TestIntegration:
    ...
    async def test_reproducibility(self, sample_claims):
        ...
        engine1 = EmergenceEngine(llm=None, params=params1)
        engine2 = EmergenceEngine(llm=None, params=params2)
