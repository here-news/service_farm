...
class EventService:
    ...
    async def examine_claims(
        ...
        if claims_added:
            ...
            # NOTE: Narrative generation is handled by LiveEvent metabolism after examine_claims returns
    ...
    async def create_root_event(self, claims: List[Claim]) -> Event:
        ...
        # Create event stub (narrative will be generated by LiveEvent after bootstrap)
        # This avoids duplicate narrative generation paths
        event = Event(
            id=temp_event.id,
            canonical_name=canonical_name,
            event_type=event_type,
            ...
            event_end=event_end,
            status='provisional',
            summary=f"{canonical_name} - narrative pending",  # Placeholder
            embedding=None  # Will be generated after narrative
        )
    ...
    async def _create_sub_event(self, parent: Event, claims: List[Claim]) -> Optional[Event]:
        ...
        # Create sub-event stub (narrative will be generated by LiveEvent after bootstrap)
        # This avoids duplicate narrative generation paths
        sub_event = Event(
    ...
    async def _generate_event_narrative_with_topology(
        ...
        if topology_context.get('contradictions'):
            ...
            for c in topology_context['contradictions'][:5]:
                text1 = c.get('text1', '')[:50] if c.get('text1') else '(text unavailable)'
                text2 = c.get('text2', '')[:50] if c.get('text2') else '(text unavailable)'
                contra_lines.append(f"  [{c['claim1_id']}] vs [{c['claim2_id']}]: \"{text1}...\" vs \"{text2}...\"")
...
{{
  "sections": [
    {{
      "topic": "overview",
      "title": "",
      "content": "2-3 paragraph opening covering core facts. Embed claim refs as [cl_xxx] or [cl_xxx][cl_yyy]. Mark entities on first mention: Name [en_xxx].",
      "claim_ids": ["cl_xxx", ...]
    }},
    {{
      "topic": "<determined by content>",
      "title": "<natural title for this aspect>",
      "content": "Detailed content for this aspect...",
      "claim_ids": ["cl_xxx", ...]
    }}
  ],
  "key_figures": [
...
SECTION STRUCTURE:
- First section MUST be topic="overview" with title="" (empty)
- Additional sections: Analyze the claims and identify natural thematic clusters
- Section titles should emerge from the actual content, not from a template
- Group related claims together and name the section after what those claims are actually about

RULES:
1. Every factual statement MUST have [cl_xxx] reference
2. For multiple claims supporting one fact: [cl_xxx][cl_yyy] format, NOT [cl_xxx, cl_yyy]
3. Mark entity names with [en_xxx] on FIRST mention only
4. Claims with plausibility ≥0.70 are established facts
5. Claims in SUPERSEDED list are "earlier reports" - include them for timeline context
...
Return ONLY valid JSON, no markdown code blocks."""
        ...
        response = await self.openai_client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content": "You are a journalist writing detailed news articles. Write LONG, comprehensive content. Each section should be 2-4 paragraphs. CRITICAL: Section titles must be SPECIFIC to this event's content - never use generic titles like 'Casualties and Impact' or 'Emergency Response'. Derive section names from what the claims actually describe."
                },
