...
class Claim:
    ...
    def entity_ids(self) -> List[str]:
        """
        Extract entity IDs (short format)

        If entities are hydrated, use those IDs.
        Otherwise, fall back to metadata JSON.
        """
        # Prefer hydrated entities (when loaded from graph)
        if self.entities is not None:
            return [str(e.id) for e in self.entities]

        # Fall back to metadata (when first extracted)
