# 30 — Architecture Principles (HERE.news)
...
## Conflict Resolution: Entropy & Independence

### The "Copying" Problem vs. Real Corroboration

**Empirical Reality**: In our analysis of 1,215 claims, we found:
- **65% of "corroborations" are just copies** (syndication, aggregation).
- Naive voting (20 vs 1) is drastically wrong if the 20 are just echoing one wire report.
- **Independence** has a perfect -1.0 correlation with effective entropy reduction.

### Our Approach: Independence-Weighted Corroboration

Instead of summing votes, we compute **Entropy (H)**:

```python
H = 1.0 - (0.49 * (effective_corroboration ** 0.30)) + ...
```

Where `effective_corroboration` is discounted by the **Independence Ratio** of the sources.

### Scenario: "2000 People Say Louvre Is in Berlin"

**Naive Voting**: 2000 > 50 → Berlin wins. (Wrong)

**Entropy Analysis**:
1.  **Berlin Claim**: 2000 sources.
    - Analysis shows 1999 are quoting 1 source.
    - Independence Ratio ≈ 0.01.
    - Effective Support = 2000 * 0.01 = **20**.
    - **Entropy remains HIGH**.

2.  **Paris Claim**: 50 sources.
    - Analysis shows 40 are independent observers (tourists, diverse media).
    - Independence Ratio ≈ 0.8.
    - Effective Support = 50 * 0.8 = **40**.
    - **Entropy drops LOW**.

**Result**: Paris wins. Not because of " votes", but because independent evidence reduced entropy more effectively.

**Industry reference**: [Information Theory (Shannon Entropy)](https://en.wikipedia.org/wiki/Entropy_(information_theory)), [Jaynesian Probability](https://en.wikipedia.org/wiki/Edwin_Thompson_Jaynes)
...
- Resource-first responses (best-shot) with background enrichment.
- Idempotency keys and replayability for every worker stage.
- Transactional outbox (or equivalent) for “write DB then emit job” without job loss.
- Stage-specific DLQs with re-drive tools and clear ownership.

### Determinism Where It Matters
- Prefer deterministic guards (time parsing, monotone counters, exact-span provenance) over model judgment.
- When models decide relations, persist the inputs/outputs so decisions are explainable and replayable.

### Versioned Contracts and Schema Discipline
- Version inter-worker payloads and keep them backwards compatible during transitions.
- Treat the database as the long-lived interface; keep derived indexes rebuildable.

### Evaluation as a First-Class Loop
- Maintain a small, curated corpus for regression testing ingestion and extraction.
- Sample events regularly for human review (usefulness, correctness, provenance clarity, update tracking).
- Track precision/recall for merge/update decisions and regressions over time.

### Security and Abuse Resistance
- Least-privilege secrets handling; rotate keys; audit access.
- Rate limits and cost controls on expensive operations (LLM calls, archive writes).
- Plan for adversarial pressure: sybils, coherence bombs, and prior capture attempts.

## Trade-Offs

### Resource-First UX vs “Completeness”
Returning a best-shot stub early means users will sometimes see partial states. The system must make “what’s pending” legible and provide refresh semantics.

### Dual-Store Complexity (Relational + Graph)
Keeping a relational source of truth alongside a graph index improves queryability, but requires:
- careful consistency boundaries,
- rebuild procedures,
- and clear rules for what is authoritative.

### Model-Driven Relations vs Epistemic Risk
LLMs are powerful for relation labeling, but can smuggle errors into the belief graph. Mitigations:
- calibration and thresholds,
- deterministic guards on numerics/time,
- auditability of every material relation decision.

### Community Signals vs Manipulation
Credits/reputation enable scalable correction, but introduce gameability. The system must:
- gate impact by reputation and stake,
- make actions traceable,
- and keep priors forkable/transparent (avoid “hidden moderation truth”).

## Success Metrics

### Epistemic Quality
- Citation coverage: percent of claims with source links (and, ideally, spans).
- Update tracking accuracy: “update vs duplicate” correctness on sampled sets.
- Merge precision/recall for events and entities (human-evaluated).
- Contradiction surfacing: contested sets are visible and navigable (UX + data).

### Reliability and Cost
- Submit latency (p95) for best-shot creation.
- Pipeline completion success rate by artifact type and source class.
- Replay/DLQ drain time targets during incidents.
- Cost per enriched artifact/event, with budgets and alerting.

### Product Value
- Time-to-understanding for a monitored topic (user-reported + behavioral).
- Alert precision and latency for watchlists.
- Retention in pilot accounts and expansion within accounts.

Prev: `docs/52.biz.operations.md`  
Next: `docs/34.arch.diagrams.md`
