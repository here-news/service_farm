# Event Formation Mechanism: From Day 1 to Complete Event
...
1. **Automatic event clustering**: ML model to detect which stories form events
2. **Automatic timeline generation**: LLM extracts timeline from claims
3. **Automatic overview synthesis**: LLM generates narrative from claims
4. **Real-time updates**: WebSocket push when new story added to event
5. **Hierarchical events**: Sub-event detection (heist → arrests → trial → verdict)
6. **Cross-event linking**: "Similar to 1911 Mona Lisa theft" with Wikidata
7. **Coherence scoring**: Measure internal consistency of claims
8. **Bias detection**: Compare framing across sources

The mechanism is in place. Now it's about automation and scale.
