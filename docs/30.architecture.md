...
The domain layer abstracts storage (PostgreSQL, Neo4j) from business logic. Workers and services operate on domain models, never raw database rows.

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   API Layer     │     │  Domain Models  │     │  Storage Layer  │
│   (FastAPI)     │ ◄── │  (Pure Python)  │ ◄── │ (Postgres/Neo4j)│
└─────────────────┘     └─────────────────┘     └─────────────────┘
      responses              dataclasses            repositories
```
...
```
backend/models/
├── domain/          # Storage-agnostic business entities
│   ├── page.py      # Page with claims, metadata
│   ├── claim.py     # Atomic fact with confidence
│   ├── entity.py    # Canonical knowledge base entry
│   ├── event.py     # Emergent event from claims
│   ├── mention.py   # Ephemeral extraction result
│   ├── source.py    # Publisher with credibility
│   └── relationships.py
│
...
| Without Domain Layer | With Domain Layer |
|---------------------|-------------------|
| `row['confidence']` scattered in code | `claim.confidence` typed access |
| Storage changes break workers | Storage changes isolated to repositories |
| Testing requires database | Domain models testable in isolation |
| Neo4j/Postgres differences leak | Unified model regardless of storage |

### Rules

1. **Workers receive domain models**, not dicts or ORM objects
2. **Repositories translate** between storage and domain
3. **API layer serializes** domain models to responses
4. **Domain models are immutable** where possible (dataclasses)
...
6. **Domain Models**: NEVER raw dicts from DB in API responses (see Part 6)
