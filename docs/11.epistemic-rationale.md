...
- Deep dive: `docs/22.theory.metabolism.md` (Jaynes-style plausibility, priors/posteriors, MaxEnt/coherence, calibration gaps, and concrete hardening steps)
...
It is **not** the technical epistemic mechanism. For the "how" (metabolism, auditability, hardening), read `docs/22.theory.metabolism.md`.
...
### The Copying Problem: 65% of "Corroboration" Is Fake

Empirical analysis of our data revealed a critical finding:
...
| Likely copies | **65%** |
| Certainty overestimation | ~3x |

**65% of news "corroborations" are copies.** When multiple outlets report the same fact, most are copying a single wire service. Naive counting ("100 sources confirm this!") overestimates certainty by roughly 3x.

This is why "more sources" doesn't equal "more truth." Independence is the key variable.

See `26.theory.calibration.md` for the full experimental validation.
...
- You can't detect **copying as corroboration** (the same wire story republished 100 times)
